...
/** Load the event schedule and display it.
 * @param {EventWrapperType} data The data to display.
*/
globalThis.displayData = data => {
    try {
        ...
        let textSchedule = '';
        textSchedule += `The Barony of Bryn Madoc — ${monthDisplay} ${data.year} Meetups: \n`;
        for(const event of data.events) {
            textSchedule += '• '
                + `${data.month}/${event.day || '?'}/${data.year} `
                + `(${event.timeStart || 'TBD'} – ${event.timeEnd || 'TBD'}): `
                + `${event.title || 'Untitled'} `
                + `(${event.location || 'Location TBD'}${event.details ? '. ' + event.details : ''}) `
                + '\n';
        }
        textSchedule += '\n'
